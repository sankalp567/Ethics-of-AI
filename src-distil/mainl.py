...
from core.kd_trainer import train_kd_epoch

TEACHER_PATH = "./checkpoints/teacherres18A_epoch_7.pth"
...
def main():
    ...
    print("Loading Teacher ResNet18...")
    ...
    print("Initializing MobileNetlarge studentmnlargeA Model...")
    ...
    model.proj = nn.Linear(960,512).to(device)
    ...
    ckp_path = os.path.join(Config.CHECKPOINT_DIR, 'MobileNetlarge_studentmnlargeA_best.pth')
    ...
    print(f"Starting Pre-training of MobileNetlarge (studentmnlargeA) from epoch {start_epoch+1} to {args.epochs}...")
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_std_acc, train_rob_acc = train_kd_epoch(
            teacher=teacher,
            student=model,
        ...
        torch.save(
            ckpt,
            f"{Config.CHECKPOINT_DIR}/studentmnlarge_kd_fd2_A_epoch_{epoch+1}.pth"
        )
        ...
        if val_rob_acc > best_rob_acc:
            ...
            torch.save(
                checkpoint,
                os.path.join(Config.CHECKPOINT_DIR, 'MobileNetlarge_studentmnlarge_kd_fd2_A_best.pth')
            )
